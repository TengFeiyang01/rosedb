...
func TestRedisDataStructure_HGet(t *testing.T) {
	...
	ok3, err := rds.HSet(utils.GetTestKey(2), []byte("field2"), v2)
	assert.True(t, ok3)
	assert.Nil(t, err)
	...
	val2, err := rds.HGet(utils.GetTestKey(2), []byte("field2"))
	assert.NotNil(t, val2)
	assert.Nil(t, err)
...
func TestRedisDataStructure_ZScore(t *testing.T) {
	...
	ok, err = rds.ZAdd(utils.GetTestKey(1), 93, []byte("val-2"))
	assert.Nil(t, err)
	assert.True(t, ok)
	...
	assert.Nil(t, err)
	assert.Equal(t, float64(93), score)
}
